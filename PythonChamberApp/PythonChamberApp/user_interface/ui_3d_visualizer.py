...
def make_cone(radius=0.1, length=0.3, color=(1, 0, 0, 1), offset=(0, 0, 0), direction=(0, 0, 1)):
    """
    Creates a cone as GLMeshItem that points in “direction” and sits at “offset”.
    Helper function for COS generation.
    """
    md = gl.MeshData.cylinder(rows=10, cols=20, radius=[radius, 0], length=length)
    m = gl.GLMeshItem(meshdata=md, smooth=True, color=color, shader="shaded", glOptions="opaque")

    # Adapt direction (Standard-cone points +Z)
    v1 = np.array([0, 0, 1])  # standard direction
    v2 = np.array(direction, dtype=float)
...
class VisualizerPyqtGraph:
    ...
    @staticmethod
    def generate_coordinate_system(axis_len=5):
        """
        Generates a 3D coordinate system with X, Y, and Z axes represented by lines and cones.
        The method returns a list of GLLinePlotItem and GLMeshItem objects that can be added to a GLViewWidget.
        The X-axis is red, the Y-axis is green, and the Z-axis is blue

        :param axis_len: Length of each axis in the coordinate system.
        :return: List of GLLinePlotItem and GLMeshItem objects representing the coordinate system. Iterate over the list and add each item to GLViewWidget.
        """
        items = []
        ax_width = 3.0

        # X-axis (red)
        items.append(gl.GLLinePlotItem(
            pos=np.array([[0, 0, 0], [axis_len, 0, 0]]),
            color=(1, 0, 0, 1), width=ax_width, antialias=True))
        items.append(make_cone(radius=10, length=25, color=(1, 0, 0, 1),
                                    offset=(axis_len, 0, 0), direction=(1, 0, 0)))

        # Y-axis (green)
        items.append(gl.GLLinePlotItem(
            pos=np.array([[0, 0, 0], [0, axis_len, 0]]),
            color=(0, 1, 0, 1), width=ax_width, antialias=True))
        items.append(make_cone(radius=10, length=25, color=(0, 1, 0, 1),
                                    offset=(0, axis_len, 0), direction=(0, 1, 0)))

        # Z-axis (blue)
        items.append(gl.GLLinePlotItem(
            pos=np.array([[0, 0, 0], [0, 0, axis_len]]),
            color=(0, 0, 1, 1), width=ax_width, antialias=True))
        items.append(make_cone(radius=10, length=25, color=(0, 0, 1, 1),
                                    offset=(0, 0, axis_len), direction=(0, 0, 1)))
