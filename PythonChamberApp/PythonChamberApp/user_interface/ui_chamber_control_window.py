...
class UI_chamber_control_window(QWidget):
    ...
    restore_position_button: QPushButton = None
    ...
    def __init_button_navigation_widget(self):
        ...
        icon_arrow_up = QIcon(os.path.join(abs_path_root, "figures/arrow-090.png"))
        icon_arrow_right = QIcon(os.path.join(abs_path_root, "figures/arrow.png"))
        icon_arrow_left = QIcon(os.path.join(abs_path_root, "figures/arrow-180.png"))
        icon_arrow_down = QIcon(os.path.join(abs_path_root, "figures/arrow-270.png"))
        icon_home = QIcon(os.path.join(abs_path_root, "figures/home.png"))
        ...
        self.restore_position_button = QPushButton("Restore App's Position from Log")
        self.restore_position_button.setToolTip("Restore current chamber position from cur_position_log.json-file.\nUse with caution!")
        main_layout.addWidget(self.restore_position_button, alignment=Qt.AlignmentFlag.AlignCenter)
    ...
    def __init_position_graph_widget(self):
        ...
        for cos in cos_list:
            ...
            cos.translate(dx=chamber_max_x, dy=0, dz=0, local=False) # translate COS to match klipper COS
            chamber_position_widget.addItem(cos)

        # create spot to display head position
        self.position_graph_head_object = Visualizer.generate_3d_antenna_object(45,10, False)
        self.position_graph_head_object.translate(dx=chamber_max_x, dy=0, dz=self.position_graph_z_head_bed_offset, local=False) # position in middle, ref to global COS
        ...
        chamber_position_widget.orbit(azim=-135, elev=5)
        chamber_position_widget.setCameraPosition(distance=1700)
    ...
    def update_live_coor_display(self, x: float, y: float, z: float):
        ...
        self.position_graph_head_object.translate(dx=-x+self.position_graph_x_max_coor, dy=y, dz=self.position_graph_z_head_bed_offset) # X Offset needed since COS visualized mirrord on x axis
        return
