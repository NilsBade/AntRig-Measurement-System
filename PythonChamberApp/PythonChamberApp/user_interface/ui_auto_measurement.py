...
class UI_auto_measurement_window(QWidget):
    ...
    #   > cubic mesh [1]
    mesh_cubic_axes_order_dropdown: QComboBox = None  # select axes order for cubic mesh
    ...
    vna_visa_timeout_lineEdit: QLineEdit = None
    ...
    def __init__(self, chamber_x_max_coor: float, chamber_y_max_coor: float, chamber_z_max_coor: float,
        ...
        configs_field_first_column = QVBoxLayout()
        ...
        configs_field_first_column.addWidget(probe_antenna_inputs_frame_widget)
        configs_field_first_column.addWidget(measurement_mesh_config_widget)

        configs_field_second_column = QVBoxLayout()
        ...
        configs_field_second_column.addWidget(vna_measurement_config_widget,alignment=Qt.AlignmentFlag.AlignTop)
        configs_field_second_column.addWidget(measurement_data_config_widget,alignment=Qt.AlignmentFlag.AlignTop)
        configs_field_second_column.addWidget(self.auto_measurement_start_button,alignment=Qt.AlignmentFlag.AlignBottom)
        # ...
        ...
        main_layout.addLayout(configs_field_first_column)
        main_layout.addLayout(configs_field_second_column)
    ...
    def __init_antenna_info_inputs_widget(self):
        ...
        self.button_set_z_zero_from_antennas.setToolTip("Calculates theoretical Zero position by sum of both antenna\n"
                                                        ...
                                                        "z-homing-sensor")
        self.button_set_z_zero_from_antennas.pressed.connect(self.update_all_mesh_plots)
    ...
    def __init_measurement_mesh_config_widget(self):
        ...
        cubic_axes_order_label = QLabel("Axes Order:")
        self.mesh_cubic_axes_order_dropdown = QComboBox()
        self.mesh_cubic_axes_order_dropdown.setToolTip("Select the order in which the axes are traversed during measurement.\n"
                                                  "Ax1, Ax2, Ax3")
        axes_order_items = [
            "X, Y, Z",
            "X, Z, Y",
        ...
        for item in axes_order_items:
            self.mesh_cubic_axes_order_dropdown.addItem(item)
        self.mesh_cubic_axes_order_dropdown.setCurrentIndex(0)  # default X,Y,Z order
        ...
        #   axes order
        cubic_mesh_config_widget_layout.addWidget(cubic_axes_order_label, 0, 0, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_axes_order_dropdown, 0, 1, 1, 2)
        #   move pattern
        cubic_mesh_config_widget_layout.addWidget(cubic_move_pattern_label, 1, 0, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_move_pattern_dropdown, 1, 1, 1, 2)
        #   X inputs
        cubic_mesh_config_widget_layout.addWidget(cubic_x_length_label, 2, 0, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_x_length_lineEdit, 2, 1, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_x_max_length_label, 2, 2, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(cubic_x_num_of_steps_label, 3, 0, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_x_num_of_steps_lineEdit, 3, 1, 1, 1)
        #   Y inputs
        cubic_mesh_config_widget_layout.addWidget(cubic_y_length_label, 4, 0, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_y_length_lineEdit, 4, 1, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_y_max_length_label, 4, 2, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(cubic_y_num_of_steps_label, 5, 0, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_y_num_of_steps_lineEdit, 5, 1, 1, 1)
        #   Z inputs
        cubic_mesh_config_widget_layout.addWidget(cubic_z_start_label, 6, 0, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_z_start_lineEdit, 6, 1, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(cubic_z_start_label_hint, 6, 2, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(cubic_z_stop_label, 7, 0, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_z_stop_lineEdit, 7, 1, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_z_max_distance_label, 7, 2, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(cubic_z_num_of_steps_label, 8, 0, 1, 1)
        cubic_mesh_config_widget_layout.addWidget(self.mesh_cubic_z_num_of_steps_lineEdit, 8, 1, 1, 1)
        ...
        self.stacked_mesh_config_widget.addWidget(more_to_come_widget)  # [2]
        ...
        self.mesh_cubic_x_length_lineEdit.editingFinished.connect(self.update_all_mesh_plots)
        self.mesh_cubic_x_num_of_steps_lineEdit.editingFinished.connect(self.update_all_mesh_plots)
        self.mesh_cubic_y_length_lineEdit.editingFinished.connect(self.update_all_mesh_plots)
        self.mesh_cubic_y_num_of_steps_lineEdit.editingFinished.connect(self.update_all_mesh_plots)
        self.mesh_cubic_z_start_lineEdit.editingFinished.connect(self.update_all_mesh_plots)
        self.mesh_cubic_z_stop_lineEdit.editingFinished.connect(self.update_all_mesh_plots)
        self.mesh_cubic_z_num_of_steps_lineEdit.editingFinished.connect(self.update_all_mesh_plots)
    ...
    def __init_vna_measurement_config_widget(self):
        ...
        # file config inputs
        visa_timeout_label = QLabel("VISA Timeout:")
        self.vna_visa_timeout_lineEdit = QLineEdit("2000")
        self.vna_visa_timeout_lineEdit.setToolTip("Timeout for VISA communication in [ms].\n"
                                                  "On timeout the app will retry its last measurement procedure.")
        visa_timeout_unit_label = QLabel("[ms]")
        ...
        inputs_layout.addWidget(visa_timeout_label,0,0,1,2, Qt.AlignmentFlag.AlignLeft)
        inputs_layout.addWidget(self.vna_visa_timeout_lineEdit,0,2,1,3, Qt.AlignmentFlag.AlignLeft)
        inputs_layout.addWidget(visa_timeout_unit_label,0,5,1,1, Qt.AlignmentFlag.AlignLeft)
        inputs_layout.addWidget(config_filepath_label,1,0,1,2, alignment=Qt.AlignmentFlag.AlignLeft)
        inputs_layout.addWidget(self.vna_config_filepath_lineEdit,1,2,1,3, alignment=Qt.AlignmentFlag.AlignLeft)
        inputs_layout.addWidget(self.vna_config_filepath_check_button,1,5,1,1, alignment=Qt.AlignmentFlag.AlignRight)
        inputs_layout.addWidget(config_filepath_hint_label,2,0,1,6, alignment=Qt.AlignmentFlag.AlignLeft)
    ...
    def __init_3d_graphic(self):
        ...
        #   COS at chamber 0,0,0
        cos_list = Visualizer.generate_coordinate_system(axis_len=530)
        for cos in cos_list:
            cos.rotate(180, 0, 1, 0, local=False)  # rotate COS to match klipper COS
            cos.translate(dx=self.chamber_x_max_coor, dy=0, dz=0, local=False) # translate COS to match klipper COS
    ...
    def update_current_zero_pos(self, new_x: float, new_y: float, new_z: float):
        ...
        Overrides the AUT height input field with new height that fits the new zero position.
        """
        ...
        if new_z is not None:
            new_AUT_height = new_z + self.chamber_z_head_bed_offset - self.get_probe_antenna_length()
            self.aut_height_lineEdit.setText(str(new_AUT_height))
    ...
    def update_3d_plot(self):
        ...
        display_z_measurement_mesh = []
        display_x_measurement_mesh = []

        # flip z orientation display in opengl graph
        for nz in mesh_info['z_vec']:
            new_z_coor_chamber_movement.append(-nz)     # points on height of base plate
        for nz in new_z_coor_chamber_movement:
            display_z_measurement_mesh.append(nz + self.get_aut_height())  # points at tip of AUT

        # offset x coordinates by workspace dimension (since opengl origin can not be moved)
        for nx in mesh_info['x_vec']:
            display_x_measurement_mesh.append(-nx + self.chamber_x_max_coor)
        display_x_antenna_coordinate = -self.current_zero_x + self.chamber_x_max_coor

        if mesh_info['tot_num_of_points'] < (51*51*51):
            # standard routine to display all points
            new_data = Visualizer.generate_point_list(tuple(display_x_measurement_mesh), mesh_info['y_vec'],
                                                      tuple(display_z_measurement_mesh))
        else:   # routine if too many points to display
            new_data = Visualizer.generate_outline_point_list(tuple(display_x_measurement_mesh), mesh_info['y_vec'],
                                                              tuple(display_z_measurement_mesh))

        self.graphic_measurement_mesh_obj.setData(pos=new_data)

        # set bed to the lowest position for mesh
        ...
        self.graphic_probe_antenna_obj.translate(dx=display_x_antenna_coordinate, dy=self.current_zero_y,
                                                 dz=self.chamber_z_head_bed_offset)
        ...
        self.graphic_aut_obj.translate(dx=display_x_antenna_coordinate, dy=self.current_zero_y, dz=lowest_z_mesh)

    def update_all_mesh_plots(self):
        """
        Firstly updates the AUT height according to the current zero position and probe length.
        Then updates all 2D and 3D plots in the auto measurement tab.
        """
        # always update AUT height according to current zero position and current probe length before updating plot
        new_AUT_height = self.current_zero_z + self.chamber_z_head_bed_offset - self.get_probe_antenna_length()
        self.aut_height_lineEdit.setText(str(new_AUT_height))
    ...
    def get_mesh_cubic_data(self):
        ...
        dict:
            {
            'axes_order' : list(string), e.g. ['x','y','z'] or ['y','x','z']
        ...
        *When used for display measurement-mesh in graph, compensate the AUT-antenna-height of zero-position in [Z]!*
        """
        #   Checking the zero position should be performed before calling this function
        if self.current_zero_x is None or self.current_zero_y is None or self.current_zero_z is None:
            raise ValueError("No complete zero position defined! Can't calculate mesh coordinates!")

        info_dict = {}
        #   get inputs
        axes_order_str = self.mesh_cubic_axes_order_dropdown.currentText()
        axes_order_list = [s.strip() for s in axes_order_str.lower().split(",")]
        ...
        #   fill info dict
        info_dict['axes_order'] = axes_order_list
    ...
    def get_vna_configuration(self):
        ...
        vna_info: dict = {
            ...
            'visa_timeout':         int, [ms], timeout for VISA communication
        ...
        if self.vna_config_selection_dropdown.currentIndex() == 1:
            ...
            vna_info['visa_timeout'] = int(self.vna_visa_timeout_lineEdit.text())
            return vna_info
