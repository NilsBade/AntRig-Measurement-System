...
class AutoMeasurement(QRunnable):
    ...
    axes_order: list = None  # order of axes in movement pattern, e.g. ['x', 'y', 'z'] or ['z', 'y', 'x'] etc.
    ...
    def __init__(self, chamber: ChamberNetworkCommands, vna: E8361RemoteGPIB, vna_info: dict, x_vec: tuple[float, ...],
                 ...
                 file_location: str, move_pattern:str, axes_order:list, file_type_json: bool = True, file_type_json_readable: bool = True):
        ...
        if self.store_as_json:
            ...
            measurement_config = {
                ...
                'axes_movement_order':       axes_order,
    ...
    def run(self):
        ...
        #   Prepare for flexible axes order and copy vectors -> axes_vectors holds the movement-relevant vectors
        axes_vectors = {
            'x': self.mesh_x_vector.copy(),
            'y': self.mesh_y_vector.copy(),
            'z': self.mesh_z_vector.copy(),
        }


        # calculate num of points and layers for progress monitoring
        num_of_points_per_layer = len(axes_vectors[self.axes_order[0]]) * len(axes_vectors[self.axes_order[1]])
        num_of_layers = len(axes_vectors[self.axes_order[2]])
        ...
        progress_dict = {
            ...
            'time_to_go': 0.0,    # time to go in [seconds] as float, initialized to 0.0 before first point is measured
        }
        ...
        if self.move_pattern == 'snake':
            ...
            axes_vectors[self.axes_order[0]] = np.flip(axes_vectors[self.axes_order[0]])
            axes_vectors[self.axes_order[1]] = np.flip(axes_vectors[self.axes_order[1]])
        #else:   # 'line-by-line'


        for a2 in axes_vectors[self.axes_order[2]]:
            layer_count += 1
            if self.move_pattern == 'snake':
                axes_vectors[self.axes_order[1]] = np.flip(axes_vectors[self.axes_order[1]])    # snake movement in ax1-direction
            # measure one layer
            for a1 in axes_vectors[self.axes_order[1]]:
                if self.move_pattern == 'snake':
                    axes_vectors[self.axes_order[0]] = np.flip(axes_vectors[self.axes_order[0]])    # snake movement in ax0-direction
                for a0 in axes_vectors[self.axes_order[0]]:
                    point_in_layer_count += 1
                    total_point_count += 1

                    # start time keeping in first iteration
                    ...
                    cur_coordinate = {
                        self.axes_order[0]: a0,
                        self.axes_order[1]: a1,
                        self.axes_order[2]: a2,
                    }
                    ...
                    while not self.measurement_iteration_success:
                        ...
                        try:
                            ...
                            self.signals.update.emit("Measurement done! Read data from VNA and write to buffer...")

                            # Calculate antenna-relative coordinates for storage in json file
                            ...
                            time.sleep(1) # sleeptime to slow down for PNA/measurement equipment to recover from error
                    ...
                    # Calc average time per point for time-to-go estimation - Timekeeping
                    self.average_time_per_point = (datetime.now() - meas_start_timestamp).total_seconds() / (total_point_count)
                    progress_dict['time_to_go'] = round(self.average_time_per_point * (total_num_of_points - total_point_count))

            # layer finished, reset point_in_layer_count
            point_in_layer_count = 0
