...
from meas_equip_interface import E8361RemoteGPIB
import numpy as np
import json
...
class ProcessController:
    ...
    __position_log_file_path: str = os.path.join(os.getcwd(), "cur_position_log.json") # hardcoded default path to log current position in case position-restore necessary
    ...
    def __init__(self):
        ...
        # self.gui_app.setWindowIcon(QIcon("PythonChamberApp/figures/AntRig_Logo_Symbol.ico"))  # not required in Win11
        ...
        self.gui_mainWindow.ui_config_window.chamber_ip_line_edit.setText("123.45.67.890")
        self.gui_mainWindow.ui_config_window.chamber_api_line_edit.setText("YoUrAPIKey1")
        ...
        self.gui_mainWindow.ui_chamber_control_window.restore_position_button.pressed.connect(
            self.chamber_control_restore_position_button_handler)
        ...
        self.gui_mainWindow.ui_auto_measurement_window.button_set_current_as_zero.pressed.connect(
            self.auto_measurement_setZero_from_current_button_handler)
    ...
    def chamber_control_update_live_position(self, pos_update_info: dict):
        ...
        # Override current position log-file
        with open(self.__position_log_file_path, 'w') as pos_log_file:
            pos_log_data = {
                'abs_x': self.__x_live,
                'abs_y': self.__y_live,
                'abs_z': self.__z_live
            }
            json.dump(pos_log_data, pos_log_file)
    ...
    def check_if_busy(self):
        """
        This function checks the process controller for running threads (e.g. Worker, AutoMeasurement, CalibrationRoutine).
    ...
    def is_zero_position_set(self) -> bool:
        """
        This function checks if zero position for AutoMeasurement is set.
        :return: True >> zero position set ; False >> zero position not set
        """
    ...
    def chamber_control_restore_position_button_handler(self):
        """
        Restores live position from python chamber app log file.
        Must be used with care, thus this method handler asks user for confirmation explicitly.

        In case no log file is found, a warning is shown.

        In case the chamber was out of operation and has an unknown position, Klipper will ignore all jog-requests
        received from the app, even though the app thinks everything is fine
        """
        # check if log file exists
        if not os.path.isfile(self.__position_log_file_path):
            self.gui_mainWindow.prompt_warning("No position log file found. Please check your configuration.",
                                               "No position log file")
            return

        dlg = QMessageBox(self.gui_mainWindow)
        dlg.setWindowTitle("Restore Chamber Position")
        dlg.setText("Do you really want to restore the current chamber position from the log file?\n"
                    "For this routine to work, the Chamber/Klipper must know its position and it should align with the last operation stored in the log file!\n"
                    "In case Klipper does not know its position at the moment, it will ignore all subsequent jog-requests sent by the app until the chamber is homed again!\n\n"
                    "Hint: You can check if the chamber is operational by accessing the octoprint web-interface.")
        dlg.setStandardButtons(QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
    ...
    def chamber_control_override_position(self):
        """
        Reads the position from the hardcoded position log file and overrides the live position accordingly.
        """
        with open(self.__position_log_file_path, 'r') as f:
            try:
                position_data = json.load(f)
            except json.JSONDecodeError as e:
                self.gui_mainWindow.prompt_warning(f"Error reading position log file: {e}", "Invalid Position Log File")
                return

        if 'abs_x' not in position_data or 'abs_y' not in position_data or 'abs_z' not in position_data:
            self.gui_mainWindow.prompt_warning("Position log file does not contain valid position data.\n"
                                               "Please check your configuration and the log file.",
                                               "Invalid Position Log File")
            return

        # override live position with values from log file
        restore_x = float(position_data['abs_x'])
        restore_y = float(position_data['abs_y'])
        restore_z = float(position_data['abs_z'])

        self.chamber_control_update_live_position({'abs_x': restore_x, 'abs_y': restore_y, 'abs_z': restore_z})
        self.gui_mainWindow.ui_chamber_control_window.append_message2console(("Position [" + str(restore_x) + ", " + str(restore_y) + ", " + str(restore_z) + "] restored from file!"))

        # enable same functions as home all
        self.gui_mainWindow.ui_chamber_control_window.control_buttons_widget.setEnabled(True)
        self.gui_mainWindow.ui_chamber_control_window.enable_go_to_controls()
        self.gui_mainWindow.ui_chamber_control_window.z_tilt_adjust_button.setEnabled(True)
        self.gui_mainWindow.ui_auto_measurement_window.enable_chamber_move_interaction()
    ...
    def auto_measurement_start_handler(self):
        ...
        if not self.is_zero_position_set():
            self.gui_mainWindow.prompt_warning("Zero position is not known. Please set a "
        ...
        #   Always update VISA communication timeout [ms]
        self.vna.pna_set_visa_timeout(vna_info['visa_timeout'])
        ...
        self.auto_measurement_process = AutoMeasurement(chamber=self.chamber, vna=self.vna, vna_info=vna_info,
                                                        ...
                                                        axes_order=mesh_info['axes_order'],
    ...
    def auto_measurement_setZero_from_current_button_handler(self):
        """
        ...
        console_msg = "Updated zero position to X:" + str(self.zero_pos_x) + " Y:" + str(self.zero_pos_y) + " Z:" + str(
            self.zero_pos_z + "; Updated AUT height according to new zero position.")
        ...
        self.gui_mainWindow.ui_auto_measurement_window.update_all_mesh_plots()
    ...
    def display_measurement_refresh_file_dropdown(self):
        ...
        if os.path.isdir(path_results_directory) is False:
            ...
            self.gui_mainWindow.prompt_warning("'results' directory was not found when searching for "
                                               ...
                                               "app your working directory is the root directory of this GIT "
