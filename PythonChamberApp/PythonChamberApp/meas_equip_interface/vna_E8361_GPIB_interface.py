...
class E8361RemoteGPIB:
    ...
    __visa_timeout = 2000  # default timeout for VISA commands in ms, can be adjusted dependent on what answer time is expected
    ...
    def connect_pna(self, resource_name: str):
        ...
        self.pna_device.timeout = self.__visa_timeout      # timeout in [ms]
        return True
    ...
    def pna_set_visa_timeout(self, timeout_ms: float):
        """
        Sets the VISA timeout for all future commands to the given value in milliseconds.
        ...
        self.pna_device.timeout = timeout_ms
        return True
    ...
    def pna_add_measurement(self, meas_name: str, parameter: list[str]):
        ...
        Once added, configuration commands can be used in combination with 'meas_name' to configure the right
        measurement on pna.
        Also creates a new Window on PNA UI and adds traces of defined measurements to the window.
    ...
    def pna_get_average_number(self, meas_name: str):
        ...
        :param meas_name: unique name of measurement
        :return: number of sweeps that are averaged for given measurement as int, False if error
        """
        meas_idx = self.get_idx_of_meas(meas_name)
        if meas_idx == -1:
            print("Error - meas_name not found in running_measurements-list!")
